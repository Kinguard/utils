...
Serial::Serial(const char *dev, speed_t speed): device(dev)
{
	port=open(dev ,O_RDWR|O_NOCTTY/*,O_SYNC*/);

	if (port<0)
	{
		throw ErrnoException("Couldn't open: "+this->device);
	}

	if ( tcgetattr(port,&savetio)<0)
	{
		close(port);
		throw ErrnoException("Couldn't save termios");
	}

	memset(&newtio, 0, sizeof(newtio));
	newtio.c_cflag = speed | CS8 | CLOCAL | CREAD | HUPCL;
	...
	{
		close(port);
		throw ErrnoException("Couldn't init new termio");
    }
...
ssize_t Serial::Read(char* buf, ssize_t len)
{
	...
	{
		throw ErrnoException("Couldn't read block");
	}
...
ssize_t Serial::Write(const char* buf, ssize_t len)
{
	...
	{
		throw ErrnoException("Couldn't write block");
	}
...
int Serial::ReadControlSignals()
{
	...
	if (ret < 0)
	{
		throw ErrnoException("Couldn't read controlsignals");
	}
...
void Serial::WriteControlSignals(int sig)
{
	...
	if (ret < 0)
	{
		throw ErrnoException("Couldn't set controlsignals");
	}
}
